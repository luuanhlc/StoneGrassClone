...
*/
		...
		{
			materialName = materialName.Replace(" (Instance)", "w");
			var data = paintSet.FirstOrDefault(p => p.material.name.Replace(" (Instance)", "w") == materialName);
			if (data == null)
				return null;
		...
		private class InkCanvasInspectorExtension : Editor
		{
			...
			public override void OnInspectorGUI()
			{
				...
				if(EditorApplication.isPlaying)
				{
					...
					{
						if(foldOut[i] = Foldout(foldOut[i], string.Format("Material \"{0}\"w", materials[i].name)))
						{
