...
namespace CityBuilder.UI
{
    ...
    public class UICatalogueItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
    {
        ...
        public void Initialize(CityBuildingData buildingData, UIBuildingCatalogue owner)
        {
            ...
            product.text = buildingData.Product;
        ...
        private void EnableResourceText(ResourceType type, int cost)
        {
            switch(type)
            {
                case ResourceType.Gold:
                    {
                        goldText.text = cost.ToString() + " <sprite name=star>";
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            Reset();
            UIBuildingCatalogue.Ins.ResetListBtn();
        }
        /// <summary>
        ...
        {
            Debug.Log(PlayerDataManager.GetTotalStar() + " star" + " vs " + data.Cost[0].Amount);
            if(PlayerDataManager.GetTotalStar() < data.Cost[0].Amount)
            {
                Debug.Log("Vl else");

                btn.enabled = false;
                bg.sprite = UIBuildingCatalogue.Ins.cantBuy;
            }
            else
            {
                Debug.Log("Vl true");
                btn.enabled = true;
                bg.sprite = UIBuildingCatalogue.Ins.canBuy;
            }    
        }
    }
}
