    ...
    public class GrassPatchActivator : MonoBehaviour
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            try
            {
                ...
                if (p != null)
                {
                    p.ChangerScale((.09f / this.gameObject.transform.localScale.x) + .02f);

                    //_hasBeeIn = p._isPainting;
                    //_hasBeeIn = true;
                    
                    p._removeCanvas.Add(_removeCanvas);
                    //p._bendCanvas.Add(_windCanvas);
                    //p._isPainting = true;
                }
            }
            catch { }
        ...
        private void OnTriggerExit(Collider other)
        {
            try
            {
                ...
                if (p != null)
                {
                    ...
                    p._removeCanvas.Remove(this._removeCanvas);
                    //p._bendCanvas.Remove(this._windCanvas);
                }
            }
            catch { }
            ...
            /*try
            {
                ...
                if (p != null)
                {
                    Debug.Log("Off" + this.gameObject.name);
                    _hasBeeIn = false;
                    _hasCopy = false;
                    p._isPainting = false;
                }
        ...
        private void Update()
        {
            if (!_hasBeeIn || !PlayerController._isMove || !infor.Ins.needcut)
                return;
            if (_isSetUp)
            {
                if (!_hasCopy)
