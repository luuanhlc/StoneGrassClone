Shader "PT/PTGrass"
{
    Properties
    {
        ...
        _Radius("Blade radius", Float) = 0.25
        _TrampleStreng("Trample Streng", Float) = 1
    }
    SubShader
    {
        Pass{
        ...
        Pass{
            ...
            void geom(triangle vertexInput IN[3], inout TriangleStream<vertexOutput> triStream)
            {
                ...
                if(alpha.x > 0.5){
                    ...
                    float2 d = IN[0].uv - _Trample.xz;
                    //float4 trample = float4(float3(normalize(d).x, normalize(d).y, normal(d).z), 0);
                    //triStream.Append(makevertex(pos + mul(transformationMatrix, float3(0, 0, height * (1 - wind))), float2(0.5, 1), IN[0].uv));
                    triStream.Append(makevertex(pos + mul(transformationMatrix, float3(0, 0, height * (1 - wind)) + float3(_Trample.x, _Trample.z, 0) * _TrampleStreng * step(length(d), _Radius) ), float2(0.5, 1), IN[0].uv));
