...
public class LuckyWheel : UICanvas
{
    ...
    public void Init()
    {
        ...
        btnFreeSpin.interactable = PlayerDataManager.GetFreeSpin();

        SetupFreeRewards();
        SetupTimeWaitFreeSpin();

        var timeLogin = PlayerDataManager.GetTimeLoginSpinVideo();
        var isNewDay = Helper.CheckNewDay(timeLogin, false);
        if (isNewDay)
        {
            PlayerDataManager.SetNumberWatchDailyVideo(GameManager.Instance.PlayerDataManager.DataLuckyWheel.NumberSpinDaily);
            PlayerDataManager.SetTimeLoginSpinVideo(DateTime.Now.ToString());
        }
    ...
    {
        if (!PlayerDataManager.GetFreeSpin())
        {

            string time = PlayerDataManager.GetTimeLoginSpinFreeWheel();
    ...
    private void SetupFreeRewards()
    {
        ...
        int numberWatchVideo = PlayerDataManager.GetNumberWatchVideoSpin();
    ...
    public void StartRotateWheel(bool isFree = true)
    {
        ...
        if (!isFree)
        {
            ...
            PlayerDataManager.SetNumberWatchDailyVideo(numberWatchDailyVideo);
        }

        int numberWatchVideo = PlayerDataManager.GetNumberWatchVideoSpin();
        numberWatchVideo++;
        PlayerDataManager.SetNumberWatchVideoSpin(numberWatchVideo);
    }
    ...
    private void OnCompleteRotate()
    {
        ...
        /*switch (dataReward.Type)
        {
            ...
            case TypeGift.HAT:
                {
                    if (PlayerDataManager.GetUnlockSkin(TypeEquipment.Hat, dataReward.IdType))
                    {
                    ...
                    else
                    {
                        ...
                        PlayerDataManager.SetUnlockSkin(TypeEquipment.Hat, dataReward.IdType);
                    }
                }
                break;
            case TypeGift.SKIN:
                {
                    if (PlayerDataManager.GetUnlockSkin(TypeEquipment.Skin, dataReward.IdType))
                    {
                        GameManager.Instance.Profile.AddGold(dataReward.NumberCoinReplace, "lucky_wheel");
                        SoundManager.Instance.PlaySoundReward();
                    ...
                    else
                    {
                        ...
                        PlayerDataManager.SetUnlockSkin(TypeEquipment.Skin, dataReward.IdType);
                    }
                }
                break;
            case TypeGift.PET:
                {
                    if (PlayerDataManager.GetUnlockSkin(TypeEquipment.Pet, dataReward.IdType))
                    {
                        GameManager.Instance.Profile.AddGold(dataReward.NumberCoinReplace, "lucky_wheel");
                        SoundManager.Instance.PlaySoundReward();
                    }
                    else
                    {
                        ...
                        GameManager.Instance.UiController.OpenPopupReward(reward, TypeDialogReward.LUCKY_WHEEL);
                        PlayerDataManager.SetUnlockSkin(TypeEquipment.Pet, dataReward.IdType);
    ...
    private void OnClickBtnFreeSpin()
    {
        ...
        PlayerDataManager.SetFreeSpin(false);
        PlayerDataManager.SetTimeLoginSpinFreeWheel(DateTime.Now.ToString());
        StartRotateWheel();
