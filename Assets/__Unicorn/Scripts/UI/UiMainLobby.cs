...
public class UiMainLobby : UICanvas
{
   ...
   // public Button BtnNoAds { get => btnNoAds; }
    ...
    void Start()
    {
        ...
        Init();     
        //StartCoroutine(run());
        MMSoundManagerMySeft.Ins.lobbySource = MMSoundManagerSoundPlayEvent.Trigger(MMSoundManagerMySeft.Ins._lobbyBg, MMSoundManager.MMSoundManagerTracks.Music, Camera.main.gameObject.transform.position);
    ...
    IEnumerator run()
    {
        ...
        //MMSoundManagerMySeft.Ins._setting.LoadSoundSettings();
        //if(MMSoundManager.MMSoundManagerTracks.Music != null)
    }
    ...
    private void Init()
    {
        ...
        txtCurentLevel.text = "Level " + SceneManager.GetSceneAt(1).name[5].ToString();
        if (SceneManager.GetSceneAt(1).name.Length == 7)
        {
            txtCurentLevel.text += SceneManager.GetSceneAt(1).name[6].ToString();
        }
    ...
    private void OnClickBtnPlay()
    {
        ...
        ShowAniHide();
        if(MMSoundManagerMySeft.Ins.lobbySource != null)
            MMSoundManagerSoundControlEvent.Trigger(MMSoundManagerSoundControlEventTypes.Stop, 0, MMSoundManagerMySeft.Ins.lobbySource);

        MMSoundManagerSoundPlayEvent.Trigger(MMSoundManagerMySeft.Ins._background, MMSoundManager.MMSoundManagerTracks.Music, Camera.main.gameObject.transform.position, true);
        CamController.Ins.Room.SetActive(true);
        CamController.Ins.BaseCame.gameObject.SetActive(true);
        CamController.Ins.LoobyCam.gameObject.SetActive(false);

        if(PlayerDataManager.GetCrrLevel() == 1)
        {
            Debug.Log("Level" + PlayerDataManager.GetCrrLevel());
            GameObject tut = Instantiate(tutHand, Canvas.transform);
            tut.SetActive(true);
        }
    ...
    {
        PlayerDataManager.SetTimeLoginOpenGift(DateTime.Now.ToString());
        ...
        *//*if (isFistOpen)
        {
            ...
            if (idSkin != -1)
            {
                ...
                PlayerDataManager.SetUnlockSkin(TypeEquipment.Skin, idSkin);
            }
            else if (idHat != -1)
            {
                RewardEndGame reward = new RewardEndGame();
                ...
                PlayerDataManager.SetUnlockSkin(TypeEquipment.Hat, reward.Id);
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                ...
                if (idSkin != -1)
                {
                    ...
                    PlayerDataManager.SetUnlockSkin(TypeEquipment.Skin, reward.Id);
                }
                else if (idHat != -1)
                {
                    RewardEndGame reward = new RewardEndGame();
                    ...
                    PlayerDataManager.SetUnlockSkin(TypeEquipment.Hat, idHat);
                }
                else
                {
                    int gold = UnityEngine.Random.Range(200, 1000);
    ...
    {
        *//*var data = GameManager.Instance.UiController.ShopCharater.DataShopSkin.GetDataShop(TypeEquipment.Skin);
        List<int> listId = new List<int>();
        for (int i = 0; i < data.Count; i++)
        {
            if (!PlayerDataManager.GetUnlockSkin(TypeEquipment.Skin, data[i].idSkin) && !data[i].typeUnlock.HasFlag(TypeUnlockSkin.SPIN))
            {
    ...
    /*private int GetIdHat()
    {
        *//*List<int> listId = new List<int>();
        for (int i = 0; i < data.Count; i++)
        {
            if (!PlayerDataManager.GetUnlockSkin(TypeEquipment.Hat, data[i].idSkin) && !data[i].typeUnlock.HasFlag(TypeUnlockSkin.SPIN))
            {
                listId.Add(data[i].idSkin);
            }
        }
    ...
    private bool IsShowGift()
    {
        ...
        string time = PlayerDataManager.GetTimeLoginOpenGift();
